...
description: Generates Lua 5.1 scripts for the Teardown game engine using API documentation from specific markdown files.
---
...
1. **Information Gathering & Mode Selection**
   ...
   - Refer to the central index file `_INDEX.md` to locate the appropriate API documentation, libraries, and tips for the selected mode.
...
2. **Implementation**
   ...
   - For **singleplayer** mods, read `docs/sp/tips/_INDEX.md` for quick-reference patterns covering entities, input, physics, UI, registry, scene queries, vector math, and audio/particles before implementing from scratch.
   - For **multiplayer** mods, leverage the **mplib** library (spawn, stats, hud, countdown, teams, tools, etc.). Read `docs/mp/mplib/_INDEX.md` for available modules and their execution contexts before implementing game-mode logic from scratch.
   - For multiplayer mods, read `docs/mp/tips/_INDEX.md` for a full quick-lookup index of MP patterns (player iteration via `Players()`, per-`playerId` input, `ServerCall`/`ClientCall`, variable scoping, optimization, and more), then consult the specific tips files as needed.
